...
def create_scatter(image_set, id_set, dim1, dim2, subject_id):
...
create_scatter(face_subspace, train_ids, 0, 1, 1)
create_scatter(face_subspace, train_ids, 1, 2, 1)
create_scatter(face_subspace, train_ids, 2, 3, 1)

create_scatter(face_subspace, train_ids, 0, 1, 2)
create_scatter(face_subspace, train_ids, 1, 2, 2)
create_scatter(face_subspace, train_ids, 2, 3, 2)
...
def k_nn(train_imgs, train_subspace, train_ids, train_recon, test_imgs, test_subspace, test_ids, test_recon):
    ...
    for i in range(len(test_imgs)):
        ...
        if test_ids[i][0] == train_ids[best_match][0]:
            ...
            plt.subplot(143)
            plt.imshow(train_imgs[j].reshape((200, 180)))
            plt.subplot(144)
            plt.imshow(train_recon[j].reshape((200, 180)))
            plt.show()
